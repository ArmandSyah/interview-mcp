...
def test_start_problem_local_writes_file(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = server_main.start_problem("0001-pair-budget-match")
...
def test_get_problem_description_uses_attempt_id(
    ...
    monkeypatch.setenv("INTERVIEW_MCP_MODE", "remote")
    started = server_main.start_problem("0001-pair-budget-match")

    description = server_main.get_problem_description(started.attempt_id)
